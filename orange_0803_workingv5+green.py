...
# file_name = 'data/images/center24.mp4'
file_name = 'pic2/orangefile0.jpg'
cam_width = 640
cam_height = 480
    ...
    def run(self):
        ...
        while True:
            ...
            if not success:  # no image captured... end the processing
                ...
                ###print(px[0],py[0])
                ###print(px,py)
                print(cam_width, 'w', cam_height, 'h')
                print(x1[0], 'center x1', y1[0], 'center y1')
                print(x2, 'center x2', y2, 'center y2')
                if x1[0] < px[0] < x2 \
                        and y1[0] < py[0] < y2:
                    #                if px[0] < (cam_width - x1[0]) and px[0] > (cam_width - x2) \
                    #                        and py[0] < (cam_height - y1[0]) and py[0] > (cam_height - y2):
                    print('laser point within box!!')
                    print(px[0], py[0], 'px / py')
                    print(cam_width - x1[0], 'cam_width - x1[0]',cam_width - x2, 'cam_width - x2',cam_height - y1[0], 'cam_height - y1[0]',cam_height - y2, 'cam_height - y2')



                else:
                    print('laser point outside box!!')
                    print(px[0], py[0], 'px(laser x) / py(laser y)')
                    print(cam_width - x1[0], 'cam_width - x1[0]', cam_width - x2, 'cam_width - x2', cam_height - y1[0],
                          'cam_height - y1[0]', cam_height - y2, 'cam_height - y2')
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        if c == 0:  # center detection
                            x_top = float(xyxy[0])
                            y_top = float(xyxy[1])
                            width = float(xyxy[2] - xyxy[0])
                            height = float(xyxy[3] - xyxy[1])
                            center_x00 = int(x_top)  # + int(width / 2)
                            center_y00 = int(y_top)  # + int(height/2)
                            center_x01 = int(x_top + width)
                            center_y10 = int(y_top + height)
                            print(center_x00, center_y00, center_x01, center_y10, 'coordinate____1')
                            ff1 = open('2box4.txt', "a")
                            ff2 = open('2point4.txt', "a")
                            # print(qwer, '1')
                            ...
                            # print(img.shape)
                            ###-----------------------------------------------mask img--------------------
                            mask = numpy.zeros(recolored_img.shape[:2], dtype="uint8")
                            px0 = 200
                            px1 = 440
                            py0 = 120
                            py1 = 360
                            ...
                            ###-----------------------------------------------mask img--------------------
                            ###--------------save for laser tracker--------------####
                            img_laser = PIL.Image.fromarray(im0, "RGB")
                            img_laser2 = img_laser.resize((int(640), int(480)))
                            fn = "pic2/orangefile" + str(count_img) + ".jpg"
                            # fn_one_image="file" + str(count_img) + ".jpg"
                            img_laser2.save(fn)
                            count_img += 1
                            # print(count_img)
                            ...
                            print(x1, y1, x2, y2, 'coordinate____2')
                            ff1.write(str(x1))
                            ff1.write(",")
                            ff1.write(str(y1))
                            ff1.write(",")
                            ...
                            dum = "\n"
                            ff1.write(dum)
                            qwer = True  ###run laser tracker
                            # print(qwer, 'laser tracking...2')
                        ...
                        if c == 2:  # pallet
                            ...
                            if hole_centerx < pallet_centerx:
                                ...
                                cv2.line(im0, (hole_centerx, hole_centery), (pallet_centerx, pallet_centery),
                                         (30, 255, 120), 1)
                                cv2.line(im0, (hole_centerx, hole_centery), (hole_centerx, pallet_y10), (30, 255, 120),
                                         1)
                                # img22 = PIL.Image.fromarray(im0, "RGB")
                                # fn = "pic3/greenfile" + str(count_img) + ".jpg"  #############################################
                                # img22.save(fn)
                                # count_img += 1
                        else:
                            pass
                ...
                if qwer == True:
                    ...
                    # tracker = LaserTracker(cam_width=640, cam_height=480, hue_min=200, hue_max=160, sat_min=155,
                    #                        sat_max=255,
                    #                        val_min=200, val_max=255, display_thresholds=params.display)
                    tracker = LaserTracker(cam_width=640, cam_height=480, hue_min=220, hue_max=160, sat_min=155,
                                           sat_max=255,
                                           val_min=200, val_max=255, display_thresholds=params.display)
                    ...
                    x1 = [x1, x2, x2, x1, x1]
                    y1 = [y1, y1, y2, y2, y1]
                    print(x1[0], 'x1_list1', x1[1],'x1_list2')
                    #print(x1[0], 'x2_list1', x1[2], 'x2_list2')
                    print(y1[0], 'y1_list1', y1[2],'y1_list2')
                    # print(y1[2], 'y2_list1', y1[2], 'y2_list2')
                    xbox = []
                    ybox = []
                    origin_mvx = list(x1)
                    for x1_i in origin_mvx:
                        xbox.append(x1_i)
                    origin_mvy = y1
                    for y1_i in origin_mvy:
                        ybox.append(y1_i)
                    plt.plot(xbox, ybox)
...
if __name__ == '__main__':
    try:
        ...
        # parser.add_argument('--source', type=str, default='pic2', help='file/dir/URL/glob, 0 for webcam')

        ...
        while True:
            ...
            if not success:  # no image captured... end the processing
                ...
                if px[0] < (cam_width-x1[0]) and px[0] > (cam_width-x2)\
                    ...
                    #GPIO.OUTPUT(~~ LED ORANGE ON)
                else:
                    print('laser point outside box!!')
                    # GPIO.OUTPUT(~~ LED ORANGE OFF)
                ff2.close()
                sys.exit(1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='palletx.pt', help='model.pt path(s)')
    parser.add_argument('--source', type=str, default=file_name, help='source')  # file/folder, 0 for webcam

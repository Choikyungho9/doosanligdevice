...
class LaserTracker(object):
    ...
    def __init__(self, cam_width=64, cam_height=480, hue_min=20, hue_max=160,
    ...
    def run(self):
        ...
        while True:
            ...
            if not success:  # no image captured... end the processing
                ...
                print(x1[0],'x',y1[0],'y')
                if (cam_width - x1[0]) < px[0] < (cam_width - x2) \
                        and (cam_height - y1[0]) < py[0] < (cam_height - y2):
                    ...
                    print(px[0],py[0],'2')
                    print(cam_width - x1[0])
                    print(cam_width - x2)
                    print(cam_height - y1[0])
                    print(cam_height - y2)
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        # if c == 1: #pallet
                        #    global trackerexe                        ###add code - code1
                        #    trackerexe=True                          ###add code - code2
                        ...
                        if c == 0:  # center
                            ...
                            #print(img.shape)
                            masked = im0[:, :, ::-1]
                            # mask = numpy.zeros(im0.shape[:2], dtype="uint8")
                            # px0 = 200
                            # px1 = 440
                            # py0 = 120
                            # py1 = 360
...
####--------------------------------------------------------------------------------------------------------------####
                            img_laser = PIL.Image.fromarray(masked, "RGB")
                            img_laser2 = img_laser.resize((int(640),int(480)))
                            fn = "pic2/file" + str(count_img) + ".jpg"
                            # fn_one_image="file" + str(count_img) + ".jpg"
                            img_laser2.save(fn)
                            count_img += 1
                            print(count_img)
                            x1 = int(xyxy[0].tolist())+10
                            ###print(x1)
                            ###print(type(x1))
                            y1 = int(xyxy[1].tolist())
                            x2 = int(xyxy[2].tolist()-xyxy[0].tolist())+10
                            y2 = int(xyxy[3].tolist()-xyxy[1].tolist())+10
...
if __name__ == '__main__':
    ...
    #parser.add_argument('--source', type=str, default='data/images/center24.mp4', help='file/dir/URL/glob, 0 for webcam')
